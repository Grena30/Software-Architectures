...
def predict_(model: GDPGrowthModel):
    ...
    return {"message": "GDP growth prediction endpoint", "input": model.model_dump(), "prediction": prediction}


@router.post("/unemployment")
def predict_unemployment(model: InflationModel):
    prediction = get_prediction("unemployment_rate", model.model_dump())
    return {"message": "Unemployment prediction endpoint", "input": model.model_dump(), "prediction": prediction}


@router.post("/inflation")
def predict_inflation(model: UnemploymentModel):
    prediction = get_prediction("inflation_rate", model.model_dump())
    return {"message": "Inflation prediction endpoint", "input": model.model_dump(), "prediction": prediction}
